...
module.exports = co.wrap(function* svgFontDump (options) {
  ...
  const data = yield loadSvgFont(options.src_font);
  const config = yield loadConfig(options.config);

  const diff = [];

  // Create output directory if doesn't exist
  yield mkdirp(options.glyphs_dir)
    .catch(() => {
      throw new Error(`Can\'t create glyph output directory ${options.glyphs_dir}`);
    });
  ...
  yield glyphs.map(function (glyph) {
    ...
    return fs.writeFile(path.join(options.glyphs_dir, filename), glyph.svg)
      .then(() => {
        diff.push(glyphOut);
      });
  });
  ...
  if (options.diff_config) {
    yield createConfigTemplate(options.diff_config, diff);
  }
});
